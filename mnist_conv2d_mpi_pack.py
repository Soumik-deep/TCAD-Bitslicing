...
def bitsliced_encrypt_mult(N,ind):
  ciphertext=[]
  x=[]
  for n in N:
    x.append(np.flipud(np.array(list(twos_complement(int(n),size))).astype(bool)))
 
  fpath="/home/user/soumik/workspace/ciXdumps/"
  #fpath1="/home/user/soumik/workspace/ciphertexts1/"
 
  for i in range(size):
    temp=[y[i] for y in x]
    fname=fpath+str("ci"+"_"+str(ind)+"_"+str(i))
...
def denseLayer(layer,input,weight_matrix,bias):
  ...
  for w in weight:
    ...
    #print("Soumik in Denselayer--Just a status check...Multiply returned ")
    #print(producttuple) 
...
def convMulAcc(layer,input,kern_matrix,kern_dim,bias,xshft,yshft):
  ...
  for w in weight:
    ...
    #	repackstart=time.time()
        ciX=bitsliced_encrypt_mult(cx,rank)
        repackend=time.time()
  ...
  mult_start=time.time()
  cmd="mpiexec -n 9 python3 bitsliced_adder_mpi.py"
  os.system(cmd)
  mult_end = time.time()
  print("ONly Multiplication takes ---"+str(int(mult_end-mult_start))+"secs")
   
   
  text_file = open("bitslicedmult.txt", "r")
  
  wind=0
  producttuple=[]
  #fpath="/home/user/soumik/workspace/ciXdumps/"
  fpath1="/home/user/soumik/workspace/Multdumps/"
  #for w in weight:
   #   fname=fpath+"mul_"+str(wind)+".txt"
    #  fmult=open(fname,"r+")
     # multresult=fmult.read()
      #producttuple=list(multresult)
      #print(producttuple)
      #result_scalar.append(producttuple)
        
        
  #lines = text_file.read()
  #print("Soumik:Debug 0")
  #print(list(lines))
  #ciphers=lines.split("\n")
  #print("Extracted Ciphers are-")
  #print(ciphers[0])
  print("Soumik:Debug 1")
  #print("length of ciphers"+str(len(ciphers)))
  #print(lines[0])
  #index=0
  #for index in range(0,len(ciphers)-1):
  	#producttuple=[]
 # 	producttuple=ciphers[index]
 # 	print("Last output before entering into next block-")
  #	print(producttuple)
  #	result_scalar.append(producttuple)
  ...
  for w in weight:
    ...
    i=0
    for i in range(size):
      #print("Length of ca[i],ca[b] are"+str(ca[i].b[0])+"  "+str(len(cb[i].b)))
        
      fname=fpath1+str("mul"+"_"+str(fid)+"_"+str(i))
      #print("The file targetis "+str(fname))
      begin=time.time()
      with open(fname, 'rb') as f:
          cx= vm.load_ciphertext(f)
          producttuple.append(cx)
    rank=rank+1       
    result_scalar.append(producttuple)  
...
def conv2D(layer,input,kern_matrix,kern_dim,bias):
 ...
 convend=time.time()
 print(f"Total runtime of the Conv2D/Convolution layer is {int(convend - convstart)} seconds")
 ...
 for i in range(conv2d_dim-1):
   for j in range(conv2d_dim-1):
     print("Inside AvgPool2D Loop i="+str(i)+" and j="+str(j))
     key=str(i)+"-"+str(j)
     validflag=0
     avgi=int(i/2)
     avgj=int(j/2)
...
for i in range(169):
  for j in range(10):
    public_weight_matrix[i][j]=random.randrange(-10, 10, 2)
...
#tracemalloc.stop()
enddense=time.time()
print(f"Total runtime of just the DenseLayer is {int(enddense-startdense)} seconds")
end = time.time()
